...
class BaseBOWModel(ABC):
    ...
    def _initialise_vocab_masks(self):
        ...
        self.vocab_masks["mid"] = self._initialise_midword_mask(
            n_logits, self.vocab_masks
        )
    ...
    def _initialise_bow_mask(self, vocab, n_logits):
        bow_subwords = set(
            idx for word, idx in vocab.items() if word[0] == self.bow_symbol
        )
    ...
    def _initialise_punct_mask(self, vocab, n_logits):
        start_of_punctuation = set(
            idx for word, idx in vocab.items() if word[0] in string.punctuation
        )
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if use_bos_symbol:
                    ...
                    tensor_input = torch.tensor(
                        [encodings["input_ids"] + [self.eos_token_id]],
                        device=self.device,
                    )
                ...
                for metric, metric_fn in self.metrics.items():
                    results = metric_fn(
                        shift_logits, shift_labels, output, tensor_input
                    )
                    all_results[metric] = np.concatenate(
                        [all_results[metric], results[offset:-1]]
                    )
            ...
            all_results["eow_fix"] = np.concatenate(
                [all_results["bow_fix"][1:], results[-1:]]
            )
